...
function submitForm (requestData) {
  ...
  const testUrl = 'https://post-tb-us.developer76543.workers.dev'
  const url = 'https://post-tb-ownggl.devmaltay01.workers.dev'
...
function finalMessage (responseText) {
  const modal = jQuery('#p-modal')
  const message = jQuery('#p-modal-message')
...
function setLoadingModal (newValue) {
  const modal = jQuery('#p-modal2')
  const message = jQuery('#p-modal2-message')
  const body = jQuery('body')
