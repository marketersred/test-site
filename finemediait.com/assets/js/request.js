...
function submitForm(requestData) {
  ...
  jQuery.ajax({
    url: 'https://post-tb-us.developer76543.workers.dev', // '/request.php',  // replaced the cf wrkr - check php for prev add
    type: 'POST',
    ...
    success: function (response) {
      ...
      if (response.status === 'success') {
        ...
        const defaultUrl = redirectMap[requestData.country] || 'https://teknogelisim.online/gizlilik.html';
...
function scrollToSection() {
  ...
  const offset =
    rect.top + window.pageYOffset - window.innerHeight / 2 + rect.height / 2 - 50
  window.scrollTo({
    top: offset,
    behavior: 'smooth'
  })
